...
def run_pipeline(dry_run=False, feeds_only=False):
    ...
    if not articles:
        logger.warning("No articles fetched. Check feed URLs and connectivity.")
        sys.exit(1)
    ...
    # Step 6: Output
    logger.info("Step 6/6: Outputting drafts")
...
def main():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print drafts to console only, don't push to Google Docs or update history",
    )
    parser.add_argument(
        "--feeds-only",
        action="store_true",
        help="Only fetch and display RSS feeds, skip generation",
    )
    args = parser.parse_args()
    run_pipeline(dry_run=args.dry_run, feeds_only=args.feeds_only)
